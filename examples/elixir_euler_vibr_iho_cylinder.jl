# A supersonic flow around a cylinder with the infinite harmonic oscillator model
...
e_int_f = T -> e_rot_cont(mass, T) + e_vibr_iho(mass, Θvibr, T)
c_int_f = T -> c_rot_cont(mass, T) + c_vibr_iho(mass, Θvibr, T)
